...
const Products = ({
  ...
  return (
    <div style={{ width: 260, paddingBottom: 15 }}>
      ...
      <div
        class="card"
        style={{ background: "#1A233A", color: "#FFFFFF" }}
        align="center"
      >
        ...
        <div class="content text-color p-2">
          <div style={{ fontSize: 23, fontWeight: "bold" }}>
            {productName.length > 13 ? (
              <div>{productName.substr(0, 14)}...</div>
            ) : (
              productName
            )}{" "}
          </div>
          <div class="description text-color">Brand : {brandName}</div>
          <Typography component={"span"} variant={"body2"}>
            ...
            <div
              className="text-color"
              style={{ fontSize: 18, paddingBottom: 8, color: "red" }}
            >
              OFFER LKR {offerPrice}.00
            </div>
          </Typography>
        </div>
